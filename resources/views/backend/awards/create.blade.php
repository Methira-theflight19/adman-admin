...
@endsection
@section ('title', trans('labels.backend.awards.management') . ' | ' . trans('labels.backend.awards.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.awards.management') }}
        <small>{{ trans('labels.backend.awards.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.awards.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-award']) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.awards.create') }}</h3>
