...
@section ('title', trans('labels.backend.galleries.management') . ' | ' . trans('labels.backend.galleries.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.galleries.management') }}
        <small>{{ trans('labels.backend.galleries.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.galleries.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-gallery','enctype' => 'multipart/form-data','files' => true]) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.galleries.create') }}</h3>
