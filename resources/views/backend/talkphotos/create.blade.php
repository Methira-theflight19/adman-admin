...
@section ('title', trans('labels.backend.talkphotos.management') . ' | ' . trans('labels.backend.talkphotos.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.talkphotos.management') }}
        <small>{{ trans('labels.backend.talkphotos.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.talkphotos.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-talkphoto','files' => true]) }}
        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.talkphotos.create') }}</h3>
