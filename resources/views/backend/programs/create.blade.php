...
@endsection
@section ('title', trans('labels.backend.programs.management') . ' | ' . trans('labels.backend.programs.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.programs.management') }}
        <small>{{ trans('labels.backend.programs.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.programs.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-program', 'files' => true]) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.programs.create') }}</h3>
