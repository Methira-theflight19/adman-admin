...
@section ('title', trans('labels.backend.activities.management') . ' | ' . trans('labels.backend.activities.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.activities.management') }}
        <small>{{ trans('labels.backend.activities.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.activities.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-activity','files' => true]) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.activities.create') }}</h3>
