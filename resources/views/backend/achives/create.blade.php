...
@endsection
@section ('title', trans('labels.backend.achives.management') . ' | ' . trans('labels.backend.achives.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.achives.management') }}
        <small>{{ trans('labels.backend.achives.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.achives.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-achive','files' => true]) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.achives.create') }}</h3>
