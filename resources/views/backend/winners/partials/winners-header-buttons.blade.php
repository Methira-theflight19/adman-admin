<!--Action Button-->
@if( Active::checkUriPattern( 'admin/winners' ) )
    <div class="btn-group">
    ...
    <ul class="dropdown-menu dropdown-menu-right" role="menu">
        <li>
            <a href="{{ route( 'admin.winners.index' ) }}">
                <i class="fa fa-list-ul"></i> {{ trans( 'menus.backend.winners.all' ) }}
            </a>
        </li>
        @permission( 'create-winner' )
            <li>
                <a href="{{ route( 'admin.winners.create' ) }}">
                    <i class="fa fa-plus"></i> {{ trans( 'menus.backend.winners.create' ) }}
                </a>
            </li>
        @endauth
    </ul>
