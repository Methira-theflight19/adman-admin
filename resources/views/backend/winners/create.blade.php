...
@endsection
@section ('title', trans('labels.backend.winners.management') . ' | ' . trans('labels.backend.winners.create'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.winners.management') }}
        <small>{{ trans('labels.backend.winners.create') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => 'admin.winners.store', 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'post', 'id' => 'create-winner','files' => true]) }}

        <div class="card box box-info">
            <div class="box-header with-border">
                <h3 class="box-title ml-3">{{ trans('labels.backend.winners.create') }}</h3>
