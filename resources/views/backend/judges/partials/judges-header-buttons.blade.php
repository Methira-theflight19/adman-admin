<!--Action Button-->
@if( Active::checkUriPattern( 'admin/judges' ) )
    <div class="btn-group">
    ...
    <ul class="dropdown-menu dropdown-menu-right" role="menu">
        <li>
            <a href="{{ route( 'admin.judges.index' ) }}">
                <i class="fa fa-list-ul"></i> {{ trans( 'menus.backend.judges.all' ) }}
            </a>
        </li>
        @permission( 'create-judge' )
            <li>
                <a href="{{ route( 'admin.judges.create' ) }}">
                    <i class="fa fa-plus"></i> {{ trans( 'menus.backend.judges.create' ) }}
                </a>
            </li>
        @endauth
    </ul>
